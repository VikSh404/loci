...
resource "proxmox_vm_qemu" "k8s-hw-worker" {

  vmid = "${ 1083 + count.index}"
    
  count = 3
  name = "k8s-hw-worker-${count.index}"
  target_node = "pve"
  full_clone  = true
  clone = "VM 9000"
...
resource "proxmox_vm_qemu" "k8s-hw-controller" {

  vmid = "${ 1080 + count.index}"
    
  count = 3
  name = "k8s-hw-controller-${count.index}"
  target_node = "pve"
  full_clone  = true
  clone = "VM 9000"
  guest_agent_ready_timeout = 60  
...
resource "proxmox_vm_qemu" "k8s-hw-proxy" {

  vmid = "${ 1099 + count.index}"
    
  count = 1
  name = "k8s-hw-proxy-${count.index}"
  target_node = "pve"
  full_clone  = true
  clone = "VM 9000"
  guest_agent_ready_timeout = 60  
