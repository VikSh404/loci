...
resource "proxmox_vm_qemu" "postgres-cluster" {

  vmid = "${ 4083 + count.index}"
    
  count = 3
  name = "postgres-cluster-${count.index}"
  target_node = "pve"
  full_clone  = true
  clone = "VM 9001"
 ...
 disk {
  ...
  os_type = "cloud-init"
  ipconfig0 = "ip=192.168.88.${43 + count.index}/24,gw=192.168.88.1"
...
resource "proxmox_vm_qemu" "haproxy" {
 ...
 disk {
  ...
  os_type = "cloud-init"
  ipconfig0 = "ip=192.168.88.${41 + count.index}/24,gw=192.168.88.1"
