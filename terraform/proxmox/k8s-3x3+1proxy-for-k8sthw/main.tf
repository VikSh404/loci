terraform {
  required_providers {
    proxmox = {
      source = "telmate/proxmox"
      version = "2.7.1"
    }
  }
}

provider "proxmox" {
    pm_tls_insecure = true
    pm_api_url = "https://192.168.88.250:8006/api2/json"
}


resource "proxmox_vm_qemu" "k8sthw-worker" {

  vmid = "${ 1083 + count.index}"
    
  count = 3
  name = "k8sthw-worker-${count.index}"
  target_node = "pve"
  full_clone  = true
  clone = "VM 9001"
  guest_agent_ready_timeout = 60  
  agent = 1
  define_connection_info = true

 disk {
    #id              = "0"
    size            = "10G"
    type            = "scsi"
    storage         = "local-lvm"        
    #storage_type    = "lvm-thin"
    #valid only with virtio disk or virtio-scsi-single controller
    #iothread        = true 
  }

  cores = 2
  sockets = 1
  memory = 1024

  ciuser = "ansible"
  cipassword = "ansible"

  sshkeys = <<EOF
  ssh-rsa  viktorshevchenko@MacBook-Air-Viktor.local
  ssh-rsa  mrv@mrv-MS-7B86
  ssh-rsa  ansible@mrv-MS-7B86
  EOF

  os_type = "cloud-init"
  ipconfig0 = "ip=192.168.88.${83 + count.index}/24,gw=192.168.88.1"

 }


resource "proxmox_vm_qemu" "k8sthw-controller" {

  vmid = "${ 1080 + count.index}"
    
  count = 3
  name = "k8sthw-controller-${count.index}"
  target_node = "pve"
  full_clone  = true
  clone = "VM 9001"
  guest_agent_ready_timeout = 60  
  agent = 1
  define_connection_info = true
 disk {
    #id              = "0"
    size            = "10G"
    type            = "scsi"
    storage         = "local-lvm"        
    #storage_type    = "lvm-thin"
    #valid only with virtio disk or virtio-scsi-single controller
    #iothread        = true 
  }

  cores = 2
  sockets = 1
  memory = 2048

  ciuser = "ansible"
  cipassword = "ansible"

  sshkeys = <<EOF
  ssh-rsa  viktorshevchenko@MacBook-Air-Viktor.local
  ssh-rsa  mrv@mrv-MS-7B86
  ssh-rsa  ansible@mrv-MS-7B86
  EOF

  os_type = "cloud-init"
  ipconfig0 = "ip=192.168.88.${80 + count.index}/24,gw=192.168.88.1"

 }



resource "proxmox_vm_qemu" "k8sthw-proxy" {

  vmid = "${ 1099 + count.index}"
    
  count = 1
  name = "k8sthw-proxy-${count.index}"
  target_node = "pve"
  full_clone  = true
  clone = "VM 9001"
  guest_agent_ready_timeout = 60  
  agent = 1
  define_connection_info = true
 disk {
    #id              = "0"
    size            = "10G"
    type            = "scsi"
    storage         = "local-lvm"        
    #storage_type    = "lvm-thin"
    #valid only with virtio disk or virtio-scsi-single controller
    #iothread        = true 
  }

  cores = 2
  sockets = 1
  memory = 1024

  ciuser = "ansible"
  cipassword = "ansible"

  sshkeys = <<EOF
  ssh-rsa  viktorshevchenko@MacBook-Air-Viktor.local
  ssh-rsa  mrv@mrv-MS-7B86
  ssh-rsa  ansible@mrv-MS-7B86
  EOF

  os_type = "cloud-init"
  ipconfig0 = "ip=192.168.88.${99 + count.index}/24,gw=192.168.88.1"

 }



