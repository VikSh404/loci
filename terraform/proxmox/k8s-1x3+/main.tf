...
resource "proxmox_vm_qemu" "k8s-worker" {
 ...
 disk {
  ...
  os_type = "cloud-init"
  ipconfig0 = "ip=192.168.88.${73 + count.index}/24,gw=192.168.88.1"
...
resource "proxmox_vm_qemu" "k8s-controller" {
 ...
 disk {
  ...
  os_type = "cloud-init"
  ipconfig0 = "ip=192.168.88.${70 + count.index}/24,gw=192.168.88.1"

 }



# resource "proxmox_vm_qemu" "k8s-proxy" {

#   vmid = "${ 1099 + count.index}"
    
#   count = 1
#   name = "k8s-proxy-${count.index}"
#   target_node = "pve"
#   full_clone  = true
#   clone = "VM 9001"
#   guest_agent_ready_timeout = 60  
...
#   os_type = "cloud-init"
#   ipconfig0 = "ip=192.168.88.${79 + count.index}/24,gw=192.168.88.1"
